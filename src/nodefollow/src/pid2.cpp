...
#define SUBSCRIBER_TOPIC_front "/path_" + std::to_string(vehicleID)
...
int main(int argc, char** argv)
{
    ...
    sleep(3);
    //初始化节点
    ros::init(argc,argv,"nodefollow2");
    ros::NodeHandle n;
    ...
    */
    ...
    //NodePID my_nodePID(pubMessage, TOLERANCE, TOLERANCE_ANGLE, MAX_SPEED, MAX_A_SPEED);

    // // 需要用message_filter容器对两个话题的数据发布进行初始化，这里不能指定回调函数
    // message_filters::Subscriber<custom_messages::VehicleStatus> sub_front(n,SUBSCRIBER_TOPIC_front,500,ros::TransportHints().tcpNoDelay());
    ...
    // // 将两个话题的数据进行同步
    // typedef message_filters::sync_policies::ApproximateTime<custom_messages::VehicleStatus, custom_messages::VehicleStatus> syncPolicy;
    ...
    while (status)
    {
        ...
        // my_nodePID->if_self_msg = false;
        // my_nodePID->if_front_msg = false;
